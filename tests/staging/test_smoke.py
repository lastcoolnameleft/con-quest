"""Staging smoke tests — run against live staging environment."""
...
BASE_URL = os.environ.get("STG_BASE_URL")

pytestmark = pytest.mark.skipif(not BASE_URL, reason="STG_BASE_URL not set")
...
class TestHealthEndpoint:
    ...
    def test_health_returns_json_with_sha(self):
        ...
        assert "sha" in data
        assert len(data["sha"]) >= 7  # at least short SHA
